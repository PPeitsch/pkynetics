...
from pkynetics.technique_analysis.dsc import (
    DSCAnalyzer, DSCExperiment, BaselineCorrector, PeakAnalyzer,
    ThermalEventDetector, CpCalculator,
)
...
def analyze_heat_capacity():
    ...
    try:
        ...
        sample_data = dsc_importer(os.path.join(CP_DATA_DIR, 'sample.txt'), manufacturer="Setaram")
        blank_data = dsc_importer(os.path.join(CP_DATA_DIR, 'zero.txt'), manufacturer="Setaram")
        ref_data = dsc_importer(os.path.join(CP_DATA_DIR, 'sapphire.txt'), manufacturer="Setaram")
        ...
        for method, result in results.items():
            ax2.plot(result.temperature, result.specific_heat,
...
def perform_dsc_analysis():
    ...
    try:
        # Import DSC data
        data = dsc_importer(os.path.join(DSC_DATA_DIR, 'sample_dsc_data.txt'), manufacturer="Setaram")
        ...
        ax1.plot(experiment.temperature, baseline_result.baseline, '--', label='Baseline')
        ax1.plot(experiment.temperature, baseline_result.corrected_data, label='Corrected')
        ax1.set_xlabel('Temperature (°C)')
        ax1.set_ylabel('Heat Flow (mW)')

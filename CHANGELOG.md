...
- `find_transformation_limits` accepts `margin="auto"`, which chooses the baseline margin from the curve instead of taking it on trust. The margin has narrow dead zones: on the Zry-4 heating run the derivative detector returns 839-936 degC for any margin from 0.18 to 0.25 and collapses to a 15 K bracket for 0.15, 0.16 and 0.17, with nothing to tell them apart from outside — the baselines inside the dead zone fit a line at R² = 0.9989 against 0.9991 outside it. `"auto"` runs the detector across the range of margins and takes the middle of the widest group giving the same answer, on the principle that the right margin is one whose answer does not depend on the margin. It works with every detector, and costs about 25 runs of one. (#26, #103)
...
### Fixed
- `SignalProcessor.smooth_signal(method="moving_average")` padded the signal with zeros, so near both ends it averaged real data with zeros and pulled any signal with an offset toward zero. The average is now centred and narrows its window near the ends instead. `method="lowess"` fitted against the sample index rather than the abscissa, which is now passed as `x`, and widened the window to at least 1 % of the points whatever `window_length` said; it now honours `window_length`. (#47)

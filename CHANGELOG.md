...
- `find_transformation_limits` accepts `margin="auto"`, which chooses the baseline margin from the curve instead of taking it on trust. Too wide a margin reaches into the transformation: on the Zry-4 heating run the derivative detector holds at 833-840 / 935-937 degC for any margin from 0.10 to 0.24 and drifts past 0.25, and nothing about a curve says in advance where that starts. `"auto"` runs the detector across the range of margins and takes the middle of the widest group giving the same answer, on the principle that the right margin is one whose answer does not depend on the margin. It works with every detector, and costs about 25 runs of one. (#26, #103)
